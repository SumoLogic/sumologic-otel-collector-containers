...
Available commands:
  ...
  gitref          Prints the git reference used to build the binary
EOF
    exit
}
...
parse_version() {
    ...
    regex="otelcol-sumo version v?([0-9]+)\.([0-9]+)\.([0-9]+)\-sumo\-([0-9]+)\-([a-f0-9]+).*"
...
# Prints the git reference used to build the binary.
git_ref() {
    echo "${git_ref}"
}
